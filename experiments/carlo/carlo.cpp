...
std::string runCommand(const std::string &scriptPath) {
  ...
  if (pid == 0) {      // Child process
    ...
    if (execl("/usr/bin/python3", "python3", relativePath(scriptPath).c_str(),
              NULL) == -1) {
...
void Server::start_listen() {
  ...
  int opt = 1;
  setsockopt(_socket_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt,
             sizeof(opt));
...
void Server::handle_request(int client_fd) {
  ...
  if (requested_file_path.find(".py") != std::string::npos) {
    ...
    if (pid == 0) {
      ...
      // Parent process
      // Close the client socket in the parent process and continue accepting
      // connections
      close(client_fd);
    }
